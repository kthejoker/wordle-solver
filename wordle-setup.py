...
# MAGIC %sql
# MAGIC CREATE WIDGET TEXT secretWordle DEFAULT "world"
...
data = ps.read_csv('/FileStore/sgb_words.txt', header = None)
...
# MAGIC %sql
# MAGIC 
# MAGIC select count(1) / (select count(1) from wordle)*100 as PercentageOf4s from wordle where match in (4)
...
wordle2 = spark.read.format('delta').load('/user/hive/warehouse/wordle')
